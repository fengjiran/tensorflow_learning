...
for root, dirs, files in os.walk(args.path):
    ...
    for file in files:
        if os.path.splitext(file)[1] in ext:
            ...
            print('handle the {}st image.'.format(i + 1))
            ...
            imsave(os.path.join(args.output, 'psv%05d.JPG' % (i * 3)), a)
            imsave(os.path.join(args.output, 'psv%05d.JPG' % (i * 3 + 1)), b)
            imsave(os.path.join(args.output, 'psv%05d.JPG' % (i * 3 + 2)), c)
