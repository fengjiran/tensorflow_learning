...
class InpaintCAModel(Model):
    ...
    def build_graph_with_losses(self, batch_data, config, training=True,
        ...
        losses['l1_loss'] = l1_alpha * \
            tf.reduce_mean(tf.abs(local_patch_batch_pos - local_patch_x1) * spatial_discounting_mask(config))
        if not config.PRETRAIN_COARSE_NETWORK:
            losses['l1_loss'] += tf.reduce_mean(tf.abs(local_patch_batch_pos -
    ...
    def test(self, img, mask, reuse=False, is_training=False):
        # generate mask, 1 represents masked point

        mask = tf.cast(mask > 127.5, tf.float32)

        batch_pos = img / 127.5 - 1.
        batch_incomplete = batch_pos * (1. - mask)
        # inpaint
        x1, x2, flow = self.build_inpaint_net(
