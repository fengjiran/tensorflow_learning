    ...
    def register_gradients(self, loss, vars):
        ...
        with tf.name_scope(self.id + '_grad'), tf.device(dev):
            ...
            grads = self._dev_opt[dev].compute_gradients(
                loss, vars, gate_gradients=tf.train.Optimizer.GATE_NONE)  # disable gating to reduce memory usage
...
class Network:
    ...
    def _init_graph(self):
        ...
        self.trainables = OrderedDict([(self.get_var_localname(var), var)
                ...
                for gpu in range(num_gpus):
                    with tf.device('/gpu:%d' % gpu):
                        ...
                        if out_shrink > 1:
                            ...
                            out_expr = [tf.nn.avg_pool(x, ksize=ksize, strides=ksize,
                                                       padding='VALID', data_format='NCHW') for x in out_expr]
    ...
    def list_layers(self):
        ...
        def recurse(scope, parent_ops, level):
            ...
            # Otherwise => interpret as a layer.
                ...
                layer_trainables = [op.outputs[0] for op in ops if op.type.startswith(
                    'Variable') and self.get_var_localname(op.name) in self.trainables]
