...
def create_celebahq(tfrecord_dir='F:\\Datasets\\celebahq',
                    celeba_dir='F:\\Datasets\\CelebA',
                    delta_dir='F:\\Datasets\\celebahq_delta',
                    num_threads=4,
                    num_tasks=100):
    print('Loading CelebA from "%s"' % celeba_dir)
    expected_images = 202599
    if len(glob.glob(os.path.join(celeba_dir, 'Img', 'img_celeba.7z', 'img_celeba', '*.jpg'))) != expected_images:
        error('Expected to find %d images' % expected_images)
    ...
    img = np.array(PIL.Image.open(os.path.join(celeba_dir, 'Img', 'img_celeba.7z', 'img_celeba', '000001.jpg')))
    md5 = hashlib.md5()
    ...
    def process_func(idx):
        ...
        orig_path = os.path.join(celeba_dir, 'Img', 'img_celeba.7z', 'img_celeba', orig_file)
        ...
        if max(pad) > border - 4:
            ...
            mask = 1.0 - np.minimum(np.minimum(np.float32(x) / pad[0], np.float32(y) / pad[1]), np.minimum(
...
def create_from_hdf5(tfrecord_dir, hdf5_filename, shuffle):
    ...
    with h5py.File(hdf5_filename, 'r') as hdf5_file:
        hdf5_data = max([value for key, value in hdf5_file.items()
                         if key.startswith('data')], key=lambda lod: lod.shape[3])
...
def execute_cmdline(argv):
    ...
    p.add_argument('tfrecord_dir', help='New dataset directory to be created')
    p.add_argument('mnist_dir', help='Directory containing MNIST')
    p.add_argument('--num_images', help='Number of composite images to create (default: 1000000)',
