...
def completion_network(images,
    ...
    with tf.variable_scope('generator'):
        ...
        bn7 = tf.nn.relu(dilated_conv7.output)
        ...
        bn8 = tf.nn.relu(dilated_conv8.output)
        ...
        bn9 = tf.nn.relu(dilated_conv9.output)
        ...
        bn10 = tf.nn.relu(dilated_conv10.output)
        ...
        bn11 = tf.nn.relu(conv11.output)  # N, 64, 64, 256
        ...
        bn12 = tf.nn.relu(conv12.output)  # N, 64, 64, 256
        ...
        bn13 = tf.nn.relu(deconv13.output)  # N, 128, 128, 128
        ...
        bn14 = tf.nn.relu(conv14.output)  # N, 128, 128, 128
        ...
        bn15 = tf.nn.relu(deconv15.output)  # N, 256, 256, 64
        ...
        bn16 = tf.nn.relu(conv16.output)  # N, 256, 256, 32
...
def markovian_discriminator(images,
                            # is_training,
                            reuse=None):
    ...
    with tf.variable_scope('markovian_discriminator', reuse=reuse):
        conv1 = Conv2dLayer(images, [3, 3, 3, 64], stride=2, name='conv1')
        # bn1_layer = BatchNormLayer(conv1.output, is_training, name='bn1')
        bn1 = tf.nn.leaky_relu(conv1.output, alpha=0.2)
        conv_layers.append(conv1)
        # bn_layers.append(bn1_layer)

        conv2 = Conv2dLayer(bn1, [3, 3, 64, 128], stride=2, name='conv2')
        # bn2_layer = BatchNormLayer(conv2.output, is_training, name='bn2')
        bn2 = tf.nn.leaky_relu(conv2.output, alpha=0.2)
        conv_layers.append(conv2)
        # bn_layers.append(bn2_layer)

        conv3 = Conv2dLayer(bn2, [3, 3, 128, 256], stride=2, name='conv3')
        # bn3_layer = BatchNormLayer(conv3.output, is_training, name='bn3')
        bn3 = tf.nn.leaky_relu(conv3.output, alpha=0.2)
        conv_layers.append(conv3)
        # bn_layers.append(bn3_layer)
