...
if platform.system() == 'Windows':
    ...
    result_path = 'E:\\TensorFlow_Learning\\Inpainting\\my_work\\imagenet_100k_l1\\images'
    model_path = 'E:\\TensorFlow_Learning\\Inpainting\\my_work\\imagenet_100k_l1\\models'
    params_path = 'E:\\TensorFlow_Learning\\Inpainting\\my_work\\imagenet_100k_l1\\params'
...
loss_recon_ori = tf.abs(recons - ground_truth)  # L1 loss
loss_recon_center = tf.reduce_mean(1e-5 + tf.reduce_mean(loss_recon_ori * mask_recon, [1, 2, 3])) / 10.
loss_recon_overlap = tf.reduce_mean(1e-5 + tf.reduce_mean(loss_recon_ori * mask_overlap, [1, 2, 3]))
...
with tf.Session() as sess:
    ...
    for epoch in range(n_epochs):
        ...
        for start, end in zip(range(0, len(trainset), batch_size),
            ...
            images_ori = list(images_ori)  # for python3
            is_none = np.sum([x is None for x in images_ori])
            if is_none > 0:
                continue

            images_crops = map(crop_random, images_ori)
            images_crops = list(images_crops)  # for python3
            # print(len(images_crops))
            ...
            if iters % 100 == 0:
                ...
                test_image_ori = list(test_image_ori)  # for python3
