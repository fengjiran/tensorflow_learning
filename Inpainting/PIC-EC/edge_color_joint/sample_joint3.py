...
class JointNetTest():
    ...
    def sample(self, images, img_grays, edges, color_domains, masks):
        ...
        refine_inputs = tf.concat([imgs_masked, color_domains, edge_outputs_merged,
                                   masks * tf.ones_like(images[:, :, :, 0:1])], axis=3)
...
output = model.sample(image, gray, edge, color, mask)
##################################################################################
