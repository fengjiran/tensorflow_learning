...
def round_int(x):
    """Round `x` and then converts to an int."""
    return int(math.floor(x + 0.5))


def shortcut(x, output_filters, stride):
    """Apply strided avg pool or zero padding to make output_filters match x."""
...
def calc_prob(curr_layer, total_layers, p_l):
    """Calculate drop prob depending on the current layer."""
...
def build_shake_drop_model(images, num_classes, is_training):
    """Build the PyramidNet Shake-Drop model.
