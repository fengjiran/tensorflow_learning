...
def float_parameter(level, maxval):
    """Help function to scale `val` between 0 and maxval .
...
def _posterize_impl(pil_img, level):
    """Apply PIL Posterize to `pil_img`."""
...
def _translate_x_impl(pil_img, level):
    """Apply PIL TranslateX to `pil_img`.
...
def _translate_y_impl(pil_img, level):
    """Apply PIL TranslateY to `pil_img`.
...
def _crop_impl(pil_img, level, interpolation=Image.BILINEAR):
    """Apply a crop to `pil_img` with the size depending on the `level`."""
...
def _enhancer_impl(enhancer):
    """Set level to be between 0.1 and 1.8 for ImageEnhance transforms of PIL."""
