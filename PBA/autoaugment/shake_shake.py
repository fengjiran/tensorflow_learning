...
def _shake_shake_skip_connection(x, output_filters, stride):
    """Add a residual connection to the filter x for the shake-shake model."""
...
def _shake_shake_block(x, output_filters, stride, is_training):
    """Build a full shake-shake sub layer."""
    batch_size = tf.shape(x)[0]

    # Generate random numbers for scaling the branches
    rand_forward = [
        tf.random_uniform([batch_size, 1, 1, 1], minval=0, maxval=1, dtype=tf.float32)
        for _ in range(2)
    ]
    rand_backward = [
        tf.random_uniform(
...
def _shake_shake_layer(x, output_filters, num_blocks, stride,
                       is_training):
    """Build many sub layers into one full layer."""
...
if __name__ == '__main__':
    x = tf.placeholder(tf.float32, [8, 32, 32, 3])
    num_classes = 10
    hparams = tf.contrib.training.HParams(shake_shake_widen_factor=2)
    is_training = True

    logits = build_shake_shake_model(x, num_classes, hparams, is_training)
