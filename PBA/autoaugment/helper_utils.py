...
def setup_loss(logits, labels):
    """Return the cross entropy for the given `logits` and `labels`."""
...
def decay_weights(cost, weight_decay_rate):
    """Calculate the loss for l2 weight decay and adds it to `cost`."""
    costs = []
...
def eval_child_model(session, model, data_loader, mode):
    """Evaluate `model` on held out data depending on `mode`.
...
def get_lr(curr_epoch, hparams, iteration=None):
    """Return the learning rate during training based on the current epoch."""
...
def run_epoch_training(session, model, data_loader, curr_epoch):
    """Run one epoch of training for the model passed in.
