...
Ops that have different behavior during training or eval have an is_training parameter.
"""
...
def variable(name, shape, dtype, initializer, trainable):
    """Return a TF variable with the passed in specifications."""
    var = tf.get_variable(
        name,
        shape=shape,
        dtype=dtype,
...
def fc(inputs,
       ...
       scope=None,
       reuse=None):
    """Create a fully connected layer applied to `inputs`.
...
if __name__ == '__main__':
    x = tf.placeholder(dtype=tf.float32, shape=(10, 256, 256, 3))
    y = zero_pad(x, 3, 5)
    # y = global_avg_pool(x)
    print(y.get_shape())
