...
from ops import *
from utils import *
from tensorflow.data import prefetch_to_device, shuffle_and_repeat, map_and_batch
...
class SAGAN(object):
    ...
    def generator(self, z, is_training=True, reuse=False):
        with tf.variable_scope("generator", reuse=reuse):
            ...
            if self.up_sample:
                ...
                x = deconv(x, channels=self.c_dim, kernel=4, stride=2,
                           use_bias=False, sn=self.sn, scope='G_deconv_logit')
                x = tanh(x)
    ...
    def build_model(self):
        ...
        if self.custom_dataset:
            ...
            gpu_device = '/gpu:0'
            inputs = inputs.apply(shuffle_and_repeat(self.dataset_num)).apply(map_and_batch(Image_Data_Class.image_processing,
                                                                                            self.batch_size, num_parallel_batches=16, drop_remainder=True)).apply(prefetch_to_device(gpu_device, self.batch_size))
